...
extension AdventOfCode2018 {
    struct Day4 {
        enum Event: Equatable {
            ...
            init(line: String) {
                if line.contains("wakes up") {
                    ...
                    self = .fallAsleep
                } else if let idStart = line.firstIndex(of: "#"),
                    let idEnd = line[line.index(after: idStart)...].firstIndex(of: " "),
                    let id = Int(String(line[line.index(after: idStart)..<idEnd])) {
                    self = .beginShift(id: id)
                } else {
                    self = .invalid
                }
            }
            ...
            static func ==(a: Event, b: Event) -> Bool {
                switch a {
                case let .beginShift(aID):
                    guard case let .beginShift(bID) = b else { return false }
                    return aID == bID
                case .fallAsleep:
                    switch b {
                    case .fallAsleep: return true
                    default: return false
                    }
                case .wakeUp:
                    switch b {
                    case .wakeUp: return true
                    default: return false
                    }
                case .invalid:
                    switch b {
        ...
        struct Entry {
            ...
            init?(date: Date, line: String, id: Int?) {
                self.date = date
                self.event = Event(line: line)
        ...
        static func parseDate<T: StringProtocol>(from line: T) -> Date? {
            guard let endIndex = line.firstIndex(of: "]") else { return nil }
        ...
        static func part1(input: String = rawInput) -> Int {
            ...
            for line in lines {
                ...
                case .wakeUp:
                    if let previous = entries.last {
                        let startMinute = Calendar.current.dateComponents(in: TimeZone(secondsFromGMT: 0)!, from: previous.date).minute!
                        let endMinute = Calendar.current.dateComponents(in: TimeZone(secondsFromGMT: 0)!, from: e.date).minute!
                        for m in startMinute..<endMinute {
                            sleepsByMinuteByID[e.id, default: [:]][m, default: 0] += 1
                        }
                    }
                default: break
                }
            ...
            for (id, sleepsByMinute) in sleepsByMinuteByID {
                let sleeps = sleepsByMinute.values.reduce(0, +)
                maxSleeps = max(maxSleeps, sleeps)
                if maxSleeps == sleeps {
                    maxId = id
                }
            }

            guard let sleeps = sleepsByMinuteByID[maxId] else { return 0 }
            maxSleeps = sleeps.values.max()!
...
extension AdventOfCode2018.Day4 {
